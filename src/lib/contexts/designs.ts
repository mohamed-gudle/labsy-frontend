...
const UploadedDesignsContext = createContext<
  UploadedDesignsContextProps | undefined
>(undefined);
 ...
 */
export const UploadedDesignsProvider: React.FC<{ children: ReactNode }> = ({
  children,
}) => {
...
export const useUploadedDesigns = () => {
  ...
  if (!ctx) {
    throw new Error(
      "useUploadedDesigns must be used within UploadedDesignsProvider"
    );
  }
  return ctx;
};
